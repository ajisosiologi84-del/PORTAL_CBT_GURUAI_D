...
export const checkDevToolsOpen = (): boolean => {
  try {
...
/**
 * Strips correct answers, isCorrect flags, and correctCategory indicators
 * from question objects before passing them to the student session.
 * This guarantees ZERO answer key data is stored in the student browser's state, memory, or DOM.
 */
export const sanitizeQuestionsForStudent = (questions: Question[]): Question[] => {
  return questions.map((q) => {
    ...
    const sanitizedCategoryStatements = q.categoryStatements
      ? q.categoryStatements.map((st) => ({
          id: st.id,
    ...
    return {
      ...
      categoryStatements: sanitizedCategoryStatements,
      explanation: '',
