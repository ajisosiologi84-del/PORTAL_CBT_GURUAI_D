...
import { BookOpen, Clock, Flag, ChevronLeft, ChevronRight, Brain, Grid, X, CheckCircle2, ShieldAlert, Lock, Megaphone, Bell, AlertTriangle, ArrowLeft, Home, Layers, ListChecks, Maximize2, Split, ShieldCheck, Wifi, WifiOff } from 'lucide-react';
...
export const TestView: React.FC<TestViewProps> = ({
  ...
  // Monitor Network Online/Offline Connectivity Status
  useEffect(() => {
    ...
    >
      ...
      <div className="bg-slate-900 text-slate-200 px-3 sm:px-6 py-1.5 text-[11px] font-semibold border-b border-slate-800 flex items-center justify-between gap-2 overflow-x-auto shrink-0 select-none shadow-xs">
        <div className="flex items-center gap-2 sm:gap-4 shrink-0">
          ...
          <span className="hidden md:inline-block text-slate-600">•</span>
          <span className={`inline-flex items-center gap-1 font-bold ${isOnline ? 'text-emerald-400' : 'text-amber-400 animate-pulse'}`}>
            {isOnline ? (
              <>
                <Wifi className="w-3.5 h-3.5 text-emerald-400" /> Mode Online
              </>
            ) : (
              <>
                <WifiOff className="w-3.5 h-3.5 text-amber-400" /> Mode Offline (Aman & Tersimpan Lokal)
              </>
            )}
          </span>
        </div>
        <div className="flex items-center gap-2 shrink-0 text-slate-400 text-[10px]">
          {!isOnline && (
            <span className="bg-amber-500/20 text-amber-300 border border-amber-500/30 px-2 py-0.5 rounded text-[10px] font-medium hidden sm:inline-block">
              ⚡ Koneksi terputus: Jawaban Anda tetap aman tersimpan di HP/Laptop
            </span>
          )}
