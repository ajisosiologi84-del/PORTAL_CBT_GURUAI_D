...
import { User, Key, LogIn, Settings, AlertCircle, KeyRound, Users, GraduationCap, BookOpen, UserCheck, FileUp, HelpCircle, CheckCircle2, Download, Sparkles, Building2, Trophy, Crown, Medal, Award, Flame, ChevronDown, ChevronUp, FileSpreadsheet, RefreshCw, FileJson, ShieldCheck, ShieldAlert, Clock, AlertTriangle, Monitor, ListChecks, Lock, ArrowLeft } from 'lucide-react';
...
export const LoginView: React.FC<LoginViewProps> = ({
  ...
  const [rightPanelTab, setRightPanelTab] = useState<'rules' | 'leaderboard'>('rules');
  ...
  return (
    <div className="flex-1 min-h-screen bg-gradient-to-br from-slate-900 via-blue-950 to-slate-900 p-3 sm:p-6 overflow-y-auto custom-scrollbar flex items-center justify-center">
      <div className="w-full max-w-6xl my-auto flex flex-col lg:flex-row gap-6 items-stretch justify-center">
        ...
        <div
          className={`bg-white rounded-3xl shadow-2xl w-full lg:w-[440px] shrink-0 overflow-hidden transition-transform duration-300 border border-slate-100 flex flex-col justify-between ${
        ...
        >
          <div>
            ...
            <div className="bg-gradient-to-r from-blue-900 via-slate-900 to-indigo-950 p-6 text-center text-white relative overflow-hidden flex flex-col items-center justify-center">
              ...
              <div className="inline-flex items-center gap-1.5 px-3 py-0.5 rounded-full text-[10px] font-black uppercase tracking-widest bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 mb-2">
                <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span>
                PORTAL UJIAN CBT AKTIF
              </div>

              <h1 className="text-xl font-black tracking-tight">PORTAL UJIAN CBT</h1>
              <p className="text-blue-200 text-xs mt-0.5 font-medium">
                Sistem Asesmen & Pelaksanaan Ujian Daring
              </p>

              {/* Sesi Ujian Info Badges */}
              <div className="mt-3 flex items-center gap-2 bg-indigo-950/80 border border-indigo-700/60 rounded-full px-3.5 py-1 text-[11px] flex-wrap justify-center">
                <span className="flex items-center gap-1">
                  <BookOpen className="w-3.5 h-3.5 text-sky-400" />
                  <span className="text-indigo-200 font-medium">Mapel:</span>
                  <span className="font-bold text-sky-300">{config.mapel || 'Sosiologi'}</span>
                </span>
                <span className="text-indigo-400 font-bold">•</span>
                <span className="flex items-center gap-1">
                  <Clock className="w-3.5 h-3.5 text-amber-400" />
                  <span className="text-indigo-200 font-medium">Durasi:</span>
                  <span className="font-bold text-amber-300">{config.duration || 60} Menit</span>
                </span>
              </div>
            </div>
            ...
            {activeMode === 'student' ? (
              <div className="bg-blue-50/80 border-b border-blue-100 px-6 py-2.5 flex items-center justify-between text-xs font-bold text-blue-900">
                <span className="flex items-center gap-1.5 uppercase tracking-wider text-[11px]">
                  <Users className="w-3.5 h-3.5 text-blue-600" /> Portal Peserta Ujian (Siswa)
                </span>
                <span className="bg-blue-600 text-white text-[10px] px-2 py-0.5 rounded-full font-black shadow-2xs">
                  SISWA
                </span>
              </div>
            ) : (
              <div className="bg-slate-900 text-white px-6 py-2.5 flex items-center justify-between text-xs font-bold border-b border-slate-800">
                <span className="flex items-center gap-1.5 uppercase tracking-wider text-[11px] text-indigo-200">
                  <ShieldCheck className="w-3.5 h-3.5 text-indigo-400" /> Mode Masuk Proktor & Pengawas
                </span>
                <button
                  type="button"
                  onClick={() => {
                  ...
                  className="bg-indigo-900/80 hover:bg-indigo-800 text-indigo-100 text-[10px] px-2.5 py-1 rounded-lg border border-indigo-700/60 flex items-center gap-1 cursor-pointer transition active:scale-95"
                >
                  <ArrowLeft className="w-3 h-3" /> Kembali ke Siswa
                </button>
              </div>
            )}
            ...
            <div className="p-5 sm:p-6 space-y-4">
              ...
              {activeMode === 'student' ? (
                /* Student Form & Package Upload */
                <div className="space-y-4">
                  {/* LANGKAH 1: UPLOAD PAKET SOAL (.JSON) */}
                  <div className="bg-gradient-to-br from-blue-50 to-indigo-50/60 border-2 border-blue-200 rounded-2xl p-3.5 space-y-2.5 shadow-2xs">
                    <div className="flex items-center justify-between">
                      <span className="font-extrabold text-xs text-blue-950 flex items-center gap-1.5 uppercase tracking-wider">
                        <span className="w-5 h-5 rounded-full bg-blue-600 text-white font-black flex items-center justify-center text-[10px] shrink-0 shadow-xs">
                          1
                        </span>
                        Upload File Paket Soal (.json)
                      </span>
                      {config.questions.length > 0 ? (
                        <span className="bg-emerald-100 text-emerald-800 text-[10px] font-black px-2 py-0.5 rounded-full border border-emerald-300 flex items-center gap-1 shadow-2xs">
                          <CheckCircle2 className="w-3 h-3 text-emerald-600" /> PAKET AKTIF
                        </span>
                      ) : (
                    ...
                    <label className="w-full bg-gradient-to-r from-amber-500 via-amber-600 to-orange-600 hover:from-amber-600 hover:to-orange-700 active:scale-[0.98] text-white font-black p-3 rounded-xl text-xs flex items-center justify-center gap-2 cursor-pointer shadow-md transition-all border border-amber-400/40">
                      <FileUp className="w-4 h-4 text-amber-100 shrink-0 animate-bounce" />
                      <span>UPLOAD / PILIH FILE PAKET SOAL (.JSON)</span>
                      <input
                        type="file"
                        accept=".json"
                    ...
                    {config.questions.length > 0 && (
                      <div className="bg-white/90 border border-blue-200/80 rounded-xl p-2.5 text-xs text-slate-700 shadow-2xs">
                        <div className="flex items-center justify-between gap-2">
                          <div className="min-w-0 flex-1">
                            <p className="font-bold text-slate-900 truncate text-xs">
                              {config.mapel || 'Sosiologi'} — {config.mapelTitle || 'Paket Ujian'}
                            </p>
                            <p className="text-[11px] text-slate-600 flex items-center gap-2 mt-0.5 flex-wrap">
                              <span><b>{config.questions.filter(q => q.isActive !== false).length}</b> Soal Siap Uji</span>
                              <span>•</span>
                              <span>Token: <b className="font-mono text-blue-700 uppercase bg-blue-50 px-1.5 py-0.5 rounded border border-blue-200">{config.examToken || 'SOS2026'}</b></span>
                            </p>
                          </div>
                        </div>
                      </div>
                  ...
                  {/* LANGKAH 2: INPUT NIS & TOKEN UNTUK MULAI UJIAN */}
                  <form onSubmit={handleStudentSubmit} className="space-y-3">
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          placeholder="Contoh: 1001 / NIS Anda"
                    ...
                    {/* Quick Info Box */}
                    <div className="bg-blue-50/70 border border-blue-200/80 rounded-xl p-2.5 text-[11px] text-slate-700 flex items-center justify-between gap-2">
                      <span className="flex items-center gap-1.5 font-medium">
                        <Clock className="w-3.5 h-3.5 text-blue-600 shrink-0" />
                        <span>Alokasi Waktu: <b className="text-slate-900">{config.duration || 60} Menit</b></span>
                      </span>
                      <span className="font-mono text-emerald-700 font-bold bg-white px-2 py-0.5 rounded border border-blue-100">
                        KKM: {config.kkm || 75}
                      </span>
                    </div>
                    ...
                    <button
                      type="submit"
                      className="w-full bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white font-black py-3.5 px-4 rounded-xl transition-all shadow-md flex items-center justify-center gap-2 active:scale-[0.98] text-sm mt-1 cursor-pointer"
                    >
                      <LogIn className="w-4.5 h-4.5" /> MULAI UJIAN SEKARANG
                    </button>
                  </form>
                </div>
              ) : (
                ...
                <>
                  <form onSubmit={handleAdminSubmit} className="space-y-3.5">
                    ...
                    >
                      {isVerifyingOnline ? (
                        ...
                        <>
                          <LogIn className="w-4 h-4" /> Masuk Pusat Kontrol Ujian
                        </>
                      )}
                    </button>
                  </form>
                  ...
                  <div className="pt-2.5 mt-2 border-t border-slate-100 flex flex-col gap-1.5">
                    ...
                    <p className="text-[10px] text-center text-slate-500 font-medium leading-relaxed">
                      Browser baru belum terdaftar akun proktor? Tarik akun langsung dari Google Sheets.
                    </p>
                  </div>

                  {/* Proctors Offline / Lab Computer Package Loader */}
                  <div className="pt-2 border-t border-slate-100 flex flex-col gap-2">
                    <label className="bg-amber-50 hover:bg-amber-100 active:bg-amber-200 text-amber-900 border border-amber-300 rounded-xl p-2 text-xs font-bold flex items-center justify-center gap-2 cursor-pointer transition active:scale-98 shadow-2xs">
                      <FileJson className="w-4 h-4 text-amber-600 shrink-0" />
                      <span>Muat Cadangan Paket Soal (.json)</span>
                      <input
                        type="file"
                        accept=".json"
                        onChange={handleImportConfigJson}
                    ...
                    <button
                      ...
                      className="text-[11px] font-semibold text-slate-500 hover:text-indigo-600 flex items-center justify-center gap-1 py-0.5 cursor-pointer"
                    >
                      <HelpCircle className="w-3.5 h-3.5 text-indigo-500" />
                      <span>Panduan Proktor & Bantuan Sinkron</span>
                    </button>
                  </div>
                </>
              )}

              {/* Discrete Proctor Access Link at Bottom */}
              <div className="pt-3 mt-2 border-t border-slate-100 text-center">
                {activeMode === 'student' ? (
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 text-[11px] font-semibold text-slate-400 hover:text-slate-600 transition-colors py-1 px-3 rounded-full hover:bg-slate-100 cursor-pointer"
                    title="Klik untuk akses masuk khusus Proktor & Guru Pengawas"
                  >
                    <Lock className="w-3 h-3 text-slate-400" />
                    <span>Akses Proktor & Pengawas Ujian</span>
                  </button>
                ) : (
                  <button
                    type="button"
                    ...
                    className="inline-flex items-center gap-1.5 text-[11px] font-bold text-blue-600 hover:text-blue-800 transition-colors py-1 px-3 rounded-full hover:bg-blue-50 cursor-pointer"
                  >
                    <ArrowLeft className="w-3 h-3" />
                    <span>Kembali ke Portal Peserta Ujian (Siswa)</span>
                  </button>
                )}
              </div>
            </div>
        ...
        {/* RIGHT CARD: PORTAL INFORMASI, TATA TERTIB & LEADERBOARD */}
        <div className="flex-1 bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 rounded-3xl p-5 sm:p-6 text-white shadow-2xl border border-indigo-500/30 flex flex-col justify-between relative overflow-hidden min-h-[500px]">
          ...
          <div>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-6 relative z-10 pb-4 border-b border-indigo-500/20">
              <div className="flex items-center gap-3">
                <div className={`p-3 rounded-2xl shadow-inner shrink-0 border ${
                  rightPanelTab === 'rules'
                ...
                <div>
                  <h2 className="font-black text-lg sm:text-xl text-white flex items-center gap-2 tracking-tight">
                    {rightPanelTab === 'rules' ? 'INFORMASI & TATA TERTIB UJIAN' : 'LEADERBOARD NILAI SISWA 🏆'}
                  </h2>
                  <p className="text-xs text-indigo-200 font-medium">
                    {rightPanelTab === 'rules'
                      ? 'Ketentuan Pelaksanaan Asesmen Daring Terstandar'
                      : `${config.mapel || 'Mata Pelajaran'} — Perolehan Nilai Teratas Peserta Ujian`}
                  </p>
                </div>
              </div>

              {/* Tab Selector: Tata Tertib vs Leaderboard */}
              <div className="flex items-center gap-1.5 bg-indigo-950/80 p-1 rounded-2xl border border-indigo-500/40">
                <button
                  type="button"
                ...
                >
                  <ListChecks className="w-3.5 h-3.5" /> Tata Tertib
                </button>
                <button
                  type="button"
                ...
                >
                  <Trophy className="w-3.5 h-3.5" /> Leaderboard ({sortedLeaderboard.length})
                </button>
              </div>
            </div>

            {/* TAB CONTENT: RULES & EXAM INFO */}
            {rightPanelTab === 'rules' && (
              <div className="space-y-4 relative z-10 animate-fade-in">
                {/* Active Exam Highlights */}
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                  <div className="bg-indigo-950/60 border border-indigo-500/30 rounded-2xl p-3">
                    <span className="text-[10px] text-indigo-300 font-bold uppercase tracking-wider block">Mata Pelajaran</span>
                    <span className="font-extrabold text-sm text-white truncate block mt-0.5">{config.mapel || 'Sosiologi'}</span>
                  </div>
                  <div className="bg-indigo-950/60 border border-indigo-500/30 rounded-2xl p-3">
                    <span className="text-[10px] text-indigo-300 font-bold uppercase tracking-wider block">Alokasi Waktu</span>
                    <span className="font-extrabold text-sm text-amber-300 block mt-0.5">{config.duration || 60} Menit</span>
                  </div>
                  <div className="bg-indigo-950/60 border border-indigo-500/30 rounded-2xl p-3">
                    <span className="text-[10px] text-indigo-300 font-bold uppercase tracking-wider block">Butir Soal Siap Uji</span>
                    <span className="font-extrabold text-sm text-emerald-300 block mt-0.5">{config.questions.filter(q => q.isActive !== false).length} Butir</span>
                  </div>
                  <div className="bg-indigo-950/60 border border-indigo-500/30 rounded-2xl p-3">
                    <span className="text-[10px] text-indigo-300 font-bold uppercase tracking-wider block">Passing Grade (KKM)</span>
                    <span className="font-extrabold text-sm text-sky-300 font-mono block mt-0.5">{config.kkm || 75} Poin</span>
                  </div>
                </div>

                {/* Official Exam Rules List */}
                <div className="bg-slate-900/80 border border-indigo-500/30 rounded-2xl p-4 sm:p-5 space-y-3">
                  <h3 className="font-bold text-xs uppercase tracking-wider text-indigo-200 flex items-center gap-2">
                    <ShieldCheck className="w-4 h-4 text-emerald-400" />
                    Ketentuan & Prosedur Pengerjaan Ujian CBT
                  </h3>

                  <div className="space-y-2.5 text-xs leading-relaxed text-slate-300">
                    <div className="flex items-start gap-2.5 bg-indigo-950/40 p-2.5 rounded-xl border border-indigo-500/20">
                      ...
                      <div>
                        <strong className="text-white block">Mode Layar Penuh (Fullscreen Wajib)</strong>
                        <span>Ujian dilaksanakan dalam mode fullscreen. Keluar dari fullscreen akan dicatat sebagai pelanggaran dan memicu alarm pengawas.</span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-start gap-2.5 bg-indigo-950/40 p-2.5 rounded-xl border border-indigo-500/20">
                      ...
                      <div>
                        <strong className="text-white block">Larangan Berpindah Tab & Aplikasi</strong>
                        <span>Sistem secara otomatis mendeteksi jika peserta membuka tab baru, browser lain, split-screen, atau tangkapan layar.</span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-start gap-2.5 bg-indigo-950/40 p-2.5 rounded-xl border border-indigo-500/20">
                      ...
                      <div>
                        <strong className="text-white block">Penyimpanan Otomatis (Autosave Realtime)</strong>
                        <span>Setiap pilihan jawaban Anda langsung tersimpan. Jika terjadi kendala jaringan atau listrik, Anda dapat melanjutkan ujian dari jawaban terakhir.</span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-start gap-2.5 bg-indigo-950/40 p-2.5 rounded-xl border border-indigo-500/20">
                      ...
                      <div>
                        <strong className="text-white block">Timer Terpusat & Autosubmit</strong>
                        <span>Penghitungan waktu berjalan secara akurat. Saat waktu ujian habis, seluruh lembar jawaban akan disubmit otomatis oleh sistem.</span>
                      </div>
                    </div>
                  </div>
                </div>
