...
import { safeSetLocalStorage, safeGetLocalStorage, getIDBData, setIDBData } from './utils/safeStorage';
...
export default function App() {
  ...
  // Restore high-capacity packages from IndexedDB if localStorage was full or bypassed
  useEffect(() => {
    getIDBData<AppConfig>(STORAGE_KEY).then((idbConfig) => {
      if (idbConfig && Array.isArray(idbConfig.questions) && idbConfig.questions.length > 0) {
        setConfig((prev) => {
          if (!prev.questions || idbConfig.questions.length > prev.questions.length) {
            return {
              ...prev,
              ...idbConfig,
            };
    ...
    getIDBData<StudentResult[]>(RESULTS_KEY).then((idbResults) => {
      if (Array.isArray(idbResults) && idbResults.length > 0) {
        setStudentResults((prev) => (prev.length === 0 ? idbResults : prev));
      }
    }).catch(() => {});
  }, []);

  // Save config to LocalStorage, IndexedDB & Firebase on updates
  const saveConfig = useCallback((newConfig: AppConfig) => {
    ...
    // 1. Save safely to local storage & IndexedDB
    try {
      safeSetLocalStorage(STORAGE_KEY, JSON.stringify(updatedConfig));
    } catch (e) {
      console.warn('Local storage write warning:', e);
    }

    // 2. Save to Firebase in separate block so quota error never blocks Firebase
    try {
      saveConfigToFirebase(updatedConfig).catch((err) => {
        console.warn('Firebase sync warning:', err);
      });
    } catch (e) {
      console.warn('Firebase save trigger error:', e);
    }
  }, []);
  ...
  const saveStudentResults = useCallback((results: StudentResult[]) => {
    ...
    try {
      safeSetLocalStorage(RESULTS_KEY, JSON.stringify(sanitizedResults));
    } catch (e) {
      console.warn('Failed to save student results:', e);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    loadConfigFromFirebase().then((remoteConfig) => {
      if (remoteConfig && Array.isArray(remoteConfig.questions) && remoteConfig.questions.length > 0) {
        ...
        setConfig((prev) => {
          ...
          try {
            safeSetLocalStorage(STORAGE_KEY, JSON.stringify(newConfig));
          } catch (e) {}
          return newConfig;
        });
      }
    ...
    loadStudentResultsFromFirebase().then((remoteResults) => {
      if (Array.isArray(remoteResults) && remoteResults.length > 0) {
        ...
        try {
          safeSetLocalStorage(RESULTS_KEY, JSON.stringify(remoteResults));
        } catch (e) {}
      }
    }).catch(() => {});
    ...
    const unsubConfig = subscribeConfigFromFirebase((remoteConfig) => {
      if (remoteConfig && Array.isArray(remoteConfig.questions) && remoteConfig.questions.length > 0) {
        ...
        try {
          safeSetLocalStorage(STORAGE_KEY, JSON.stringify(merged));
        } catch (e) {}
      }
    });
  ...
  const masterExamQuestionsRef = useRef<Question[]>([]);
  ...
  const handleStartTest = () => {
    ...
    masterExamQuestionsRef.current = preparedQuestions;
    const studentSafeQuestions = sanitizeQuestionsForStudent(preparedQuestions);

    setActiveQuestions(studentSafeQuestions);
  ...
  useEffect(() => {
    ...
    // Periodic Check for Split Screen, DevTools Open & Fullscreen Enforcement (Every 2 seconds)
    const intervalChecker = setInterval(() => {
      ...
      if (checkDevToolsOpen() && !isWarningModalOpenRef.current) {
        handleTriggerWarning('🚨 Peringatan Keamanan: Inspect Element / DevTools Terdeteksi Ditampilkan!');
      }
    }, 2000);
  ...
  const processSubmission = (autoDownload = false, customReason = '') => {
    ...
    activeQuestions.forEach((q, idx) => {
      const masterQ = masterExamQuestionsRef.current.find((mq) => mq.id === q.id) ||
                      config.questions.find((mq) => mq.id === q.id) || q;
      const qPoin = typeof masterQ.poin === 'number' && masterQ.poin > 0 ? masterQ.poin : 10;
      totalMaxPoints += qPoin;

      const userAns = userAnswers[idx];
      if (isQuestionAnswerCorrect(masterQ, userAns)) {
        correct++;
